...
| FileBrowser | 左侧 | 纯文件管理：当前 Repository（Vault）的文件树、仓库切换器、管理仓库入口；面板边缘负责折叠/展开 | 是 | 涉及 Project 概念或 Project 管理 UI |
| Editor | 中央 | AI-native 工作台：md 编辑/预览、AI 辅助润色、Knowledge 提取、对话式协作 | 否 | 替代右侧面板的 Chat 或 Knowledge 功能 |
| RightPanel | 右侧 | 多功能面板：Chat / Knowledge / Outline / Graph 视图，顶部图标切换；面板边缘负责折叠/展开 | 是 | 承载文件管理或设置页面 |
...
- Manages panel visibility (left panel expanded/collapsed, right panel expanded/collapsed) through panel edge controls.
...
- Must not toggle FileBrowser or RightPanel collapse; panel collapse belongs to panel edge controls.
...
- Collapse/expand is controlled by the visible panel edge button, not by VerticalToolbar.
...
- Collapse/expand is controlled by the visible panel edge button; the button remains available in both states.

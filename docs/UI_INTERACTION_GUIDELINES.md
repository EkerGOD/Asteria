...
| VerticalToolbar | 最左侧 | 快捷操作图标（命令面板入口等），底部固定设置按钮 ⚙️ | 否 | 承载页面导航、业务内容或文件浏览器折叠入口 |
| FileBrowser | 左侧 | 纯文件管理：当前 Repository（Vault）的文件树、仓库切换器、管理仓库入口；面板顶部工具区负责折叠/展开 | 是 | 涉及 Project 概念或 Project 管理 UI |
| Editor | 中央 | AI-native 工作台：md 编辑/预览、AI 辅助润色、Knowledge 提取、对话式协作 | 否 | 替代右侧面板的 Chat 或 Knowledge 功能 |
| RightPanel | 右侧 | 多功能面板：Chat / Knowledge / Outline / Graph 视图，顶部图标切换；顶部 tab 区负责折叠/展开 | 是 | 承载文件管理或设置页面 |
...
- Manages panel visibility (left panel expanded/collapsed, right panel expanded/collapsed) through controls integrated into each panel's top toolbar.
- Manages left and right panel widths with constrained drag resize while preserving usable Editor space.
...
- Main section: quick action buttons (command palette placeholder, extensible).
- Bottom section: settings gear button ⚙️, fixed at bottom.
- All icon buttons must have `aria-label` and title tooltip.
- Must not toggle FileBrowser or RightPanel collapse; panel collapse belongs to controls integrated into each panel.
...
- Collapsible left panel, ~200-360px wide when expanded, adjustable by dragging the boundary next to Editor.
- Collapse/expand is controlled by the visible button in the FileBrowser top toolbar, not by VerticalToolbar.
...
- Collapsible right panel, ~280-520px wide when expanded, adjustable by dragging the boundary next to Editor.
...
- Collapse/expand is controlled by the visible button in the RightPanel tab row; the button remains available in both states.

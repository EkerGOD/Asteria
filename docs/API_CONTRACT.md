...
POST /api/knowledge-units/{id}/tags
DELETE /api/knowledge-units/{id}/tags/{tag_id}

`GET /api/knowledge-units` supports optional filters:

- `project_id`: return active knowledge units for one project.
- `tag_slugs`: repeatable tag slug filter.
- `q`: keyword search across title, content, and source URI.
- `include_archived`: include archived units when `true`.
...
Tag creation accepts a required `name` and optional `color`. Tag slugs are
generated by the backend and are used by Knowledge filters.
...
Keyword search for active Knowledge units is exposed through
`GET /api/knowledge-units?q=...` so the Knowledge list can combine keyword, tag,
project, and archive filters in one request.

...
- [ ] Markdown 渲染的标题、列表、代码块、表格、链接、粗体、斜体在浅色和深色主题下均可读
- [ ] 代码块在浅色/深色主题下语法高亮色彩对比度充足
- [ ] 打开对话时先显示 loading 状态，内容就绪后平滑过渡展示，无滚动闪现
- [ ] 用户消息气泡下方 hover 时显示复制和编辑按钮，鼠标移出后隐藏
- [ ] AI 回复气泡下方始终显示复制和重试按钮
- [ ] Chat 列表项 hover 时背景色改变但尺寸不变，列表不发生抖动
- [ ] Chat 列表项 selected 状态仅通过颜色区分，尺寸不变
- [ ] `cd apps/desktop && npm run typecheck` 通过
- [ ] `cd apps/desktop && npm run lint` 通过

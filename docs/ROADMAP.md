...
5. [Bug] 发送消息后需等 API 返回才显示用户消息——缺少乐观更新
6. [Bug] 切换视图后返回 Chat，滚动定位行为不稳定——isInitialScrollRef 存在竞态
7. [UX] 对话列表操作按钮（重命名、归档、删除）分散为独立按钮，应合并到 `...` 菜单
...
- handleSend 中立即将用户消息追加到消息列表（乐观更新），API 返回后再追加 assistant message
- 排查并修复 isInitialScrollRef 竞态条件，确保切换视图后首次加载始终 instant 滚动
- 对话列表每项右侧添加 `...` 按钮，点击展开下拉菜单（Rename、Archive、Delete）
- 所有动作按钮使用 Codicons

验收标准：

- [x] AI 回复中的 Markdown 格式正确渲染（标题、列表、代码块、粗体、斜体、链接、表格）
- [x] 代码块有语法高亮
- [x] 用户消息 hover 时显示复制和编辑按钮，鼠标移出后隐藏
- [x] AI 回复底部始终显示复制和重试按钮
- [x] 复制按钮可复制消息纯文本内容到剪贴板
- [x] 输入框高度随文本内容自适应增长
- [x] 输入框达到最大高度后出现垂直滚动条
- [x] 输入框高度变化有平滑过渡动画
- [x] 用户发送消息后该消息立即显示在消息列表中，不等待 AI 回复
- [x] 切换视图后返回 Chat，消息列表始终直接定位到底部，不出现自上而下的滚动动画
- [x] 对话列表每项右侧有 `...` 按钮，点击展开下拉菜单
- [x] 下拉菜单包含 Rename、Archive、Delete 操作，点击外部或选中后自动关闭
- [x] `cd apps/desktop && npm run typecheck` 通过
- [x] `cd apps/desktop && npm run lint` 通过
...
- 不修改 Provider 架构（Provider 模型重构留到 v0.10.0）
- 不修改后端 AI 调用流程（仅消费已有的模型列表和 token 信息）
- 不修改数据库 schema
- 仅做消息元数据展示配置和 Chat 模型快捷选择 UI

解决的问题：

1. [Feature] AI 回复底部仅显示模型名，缺少时间戳、token 消耗等元数据
2. [Feature] 缺少消息元数据显示配置入口，用户无法选择展示哪些信息
3. [Feature] Chat 输入区无法快捷切换模型，需进入 Settings 才能更改

方案：

- 后端 sendChat 响应新增 token 消耗和响应延迟字段
- Settings 新增 Display / 消息显示选项卡，提供元数据勾选项（模型名、时间戳、token 消耗、响应延迟）
- 元数据显示配置本地持久化
- 发送按钮旁新增模型切换下拉按钮，列出所有已配置的 chat 模型
- 切换模型后当前对话的后续消息使用新模型

验收标准：

- [x] AI 回复底部可按用户配置显示模型名、时间戳、token 消耗、响应延迟
- [x] Settings 中有消息显示配置区域，支持勾选/取消各项元数据
- [x] 元数据显示配置本地持久化，重启后保持
- [x] 发送按钮旁有模型切换下拉按钮
- [x] 模型下拉列出所有已配置的 chat 模型
- [x] 切换模型后新消息使用所选模型
- [x] 切换模型不影响历史消息的模型标记
- [x] `cd apps/api && pytest` 通过
- [x] `cd apps/desktop && npm run typecheck` 通过
...
- 不修改数据库 schema
- 不新增依赖
- 不改动 Provider、RAG、Knowledge 业务能力
- 仅修改 Chat 视图相关前端组件和样式

解决的问题：

1. [Bug] Markdown 渲染内容颜色不跟随主题变化，深色主题下文本几乎不可见
2. [Bug] 打开对话时 scrollToBottom 存在可见的滚动闪现，体验生硬
3. [UX] 消息操作按钮（复制、编辑、重试）位于消息气泡内部，应统一移到气泡下方
4. [Bug] Chat 列表项 hover 时背景会变大导致列表抖动，应只改变颜色不改变尺寸
5. [Bug] 模型切换按钮中模型名称宽度限制过紧（max-w-[100px]），长模型名被截断无法完整阅读

方案：

- Markdown 渲染组件中的文字、代码块、表格等元素改用 Tailwind 主题 CSS 变量，确保浅色/深色主题均可读
- 打开对话时先展示 loading skeleton，消息列表渲染并 scrollToBottom 执行完毕后再显示内容
- 用户消息操作按钮（复制、编辑）移到气泡下方，hover 时显示
- AI 回复操作按钮（复制、重试）移到气泡下方，始终显示
- Chat 列表项 hover / selected / focus 状态使用背景色区分，移除所有尺寸变化（padding、margin、scale、border-width 等）
- 按钮使用 Codicons，保持与现有图标系统一致
- 模型切换按钮移除固定 max-w 限制，让模型名自然延展；下拉选项添加 title 属性支持悬停查看全名

验收标准：

- [x] Markdown 渲染的标题、列表、代码块、表格、链接、粗体、斜体在浅色和深色主题下均可读
- [x] 代码块在浅色/深色主题下语法高亮色彩对比度充足
- [x] 打开对话时先显示 loading 状态，内容就绪后平滑过渡展示，无滚动闪现
- [x] 用户消息气泡下方 hover 时显示复制和编辑按钮，鼠标移出后隐藏
- [x] AI 回复气泡下方始终显示复制和重试按钮
- [x] Chat 列表项 hover 时背景色改变但尺寸不变，列表不发生抖动
- [x] Chat 列表项 selected 状态仅通过颜色区分，尺寸不变
- [x] 模型切换按钮中模型名不再被截断，常见长度模型名可完整显示
- [x] 模型下拉选项中过长名称截断时有 title 属性可悬停查看全名
- [x] `cd apps/desktop && npm run typecheck` 通过
- [x] `cd apps/desktop && npm run lint` 通过

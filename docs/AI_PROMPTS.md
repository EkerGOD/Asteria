...
1. 逐条复述你对每条笔记的理解，指出：
   - 你判断的根因（是 bug 还是架构问题，是交互设计缺失还是数据模型不足）。
   - 如果笔记描述模糊，向我提问澄清。
2. 识别笔记之间的关联——哪些问题可能是同一个底层原因导致的。
3. 检查 docs/ROADMAP.md 中已有的 planned 版本，哪些笔记可以合并到已有版本。
4. 提出初步的版本划分建议（合并/新建），说明优先级理由。

不要生成 Roadmap 文件。等我确认理解正确后，我说"可以生成"再进入第二阶段。
```

### 第二阶段：生成 Roadmap

```
理解确认完毕，现在生成 Roadmap 草案，追加到 docs/ROADMAP.md。
